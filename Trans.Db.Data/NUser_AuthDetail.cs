...
{
	/// <summary>
	/// 数据库操作:NUser_AuthDetail
	/// </summary>
	public class NUser_AuthDetail
	{
		/// <summary>
		/// 默认查询是否用只读串
		/// </summary>
		...
		public static DataTable GetTable(string sqlWhere = "", string sqlSort = "", string sqlCols = "*", object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			...
			StringBuilder sql = new StringBuilder("SELECT " + sqlCols + " FROM [NUser_AuthDetail](NOLOCK)");
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_AuthDetail> GetList(string sqlWhere = "", string sqlSort = "", object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			DataTable table = GetTable(sqlWhere, sqlSort, "*", ParamsList, ReadOnlyDataSource);
			List<Trans.Db.Model.NUser_AuthDetail> rtnObjList = new List<Trans.Db.Model.NUser_AuthDetail>();
			if (table != null && table.Rows.Count > 0)
			{
				foreach (DataRow dr in table.Rows)
				{
					rtnObjList.Add(new Trans.Db.Model.NUser_AuthDetail(dr));
				}
			}
			return rtnObjList;
		}
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_AuthDetail> GetList(string sqlWhere, string sqlSort, string sqlCols, int pageIndex, int pageSize, out int recordCount, object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			long LRecordCount = 0;
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_AuthDetail> GetList(string sqlWhere, string sqlSort, string sqlCols, int pageIndex, int pageSize, out long recordCount, object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			long LRecordCount = 0;
			DataTable table = GetTable(sqlWhere, sqlSort, sqlCols, pageIndex, pageSize, out LRecordCount, ParamsList, ReadOnlyDataSource);
		...
		{
			DataTable table = new DataTable();
			string sql = "SELECT * FROM NUser_AuthDetail(NOLOCK) WHERE [ID]=@ID";
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_AuthDetail> GetListByID(Int32 _ID, bool ReadOnlyDataSource = true)
		{
			DataTable table = GetTableByID(_ID, ReadOnlyDataSource);
			List<Trans.Db.Model.NUser_AuthDetail> objList = new List<Trans.Db.Model.NUser_AuthDetail>();
			if (table != null && table.Rows.Count > 0)
			{
				foreach (DataRow dr in table.Rows)
				{
					objList.Add(new Trans.Db.Model.NUser_AuthDetail(dr));
				}
			}
			return objList;
		}
		...
		{
			StringBuilder sql = new StringBuilder("SELECT TOP 1 " + sqlCol + " FROM NUser_AuthDetail(NOLOCK)");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(" WHERE ").Append(sqlWhere);
			}
		...
		{
			StringBuilder sql = new StringBuilder("SELECT COUNT(1) FROM NUser_AuthDetail(NOLOCK) WHERE 1=1");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
		...
		public static DataTable GetTable(string sqlWhere, string sqlSort, string sqlCols, int pageIndex, int pageSize, out long recordCount, object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			...
			{
				sqlCols = "ID,UserID,DetailName,AllowList,AllowSync,AllowRoll,SiteName,IsDir,IsDel,CreateTime";
			}
			...
			sql.Append("FROM NUser_AuthDetail(NOLOCK)");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(" WHERE ").Append(sqlWhere);
			}
			...
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(";SELECT COUNT(1) FROM NUser_AuthDetail(NOLOCK) WHERE ").Append(sqlWhere);
			}
			else
			{
				sql.Append(";SELECT COUNT(1) FROM NUser_AuthDetail(NOLOCK)");
			}
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static Trans.Db.Model.NUser_AuthDetail Get(string sqlWhere, string sqlSort = "", object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			StringBuilder sql = new StringBuilder("SELECT TOP 1 * FROM NUser_AuthDetail(NOLOCK)");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(" WHERE ").Append(sqlWhere);
			}
			...
			{
				return new Trans.Db.Model.NUser_AuthDetail(DT.Rows[0]);
			}
			else
			{
				return new Trans.Db.Model.NUser_AuthDetail();
			}
		}
		#endregion
		#endregion
		...
		public static int DBDelete(string sqlWhere, object[] ParamsList = null, bool IsRowLock = true)
		{
			...
			if (IsRowLock)
			{
				deleteSql = "DELETE FROM NUser_AuthDetail WITH(ROWLOCK) WHERE ";
			}
			else
			{
				deleteSql = "DELETE FROM NUser_AuthDetail WHERE ";
			}
		...
		{
			StringBuilder sql = new StringBuilder("UPDATE NUser_AuthDetail SET IsDel=1 WHERE ");
			sql.Append(sqlWhere);
			SqlCommand cmd = BuildCommand(sql.ToString(), ParamsList);
			return new Trans.Db.DBUtility.DBHelper(false).ExecNonQuery(cmd);
		}
		...
		{
			StringBuilder sql = new StringBuilder("UPDATE NUser_AuthDetail SET ");
		...
		/// </summary>
		/// <param name="NUser_AuthDetail_obj"></param>
		/// <returns></returns>
		public static int Update(Trans.Db.Model.NUser_AuthDetail NUser_AuthDetail_obj)
		{
			if(NUser_AuthDetail_obj.ModifiedColumns.Count <= 0)
			{
				return -1;
			}
			StringBuilder sqlSet = new StringBuilder();
			StringBuilder sqlWhere = new StringBuilder();
			string sql = "UPDATE NUser_AuthDetail SET {0} WHERE {1}";
			SqlCommand cmd = new SqlCommand();
			cmd.Parameters.AddWithValue("@ID", NUser_AuthDetail_obj.ID);
			...
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[UserID]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[DetailName]"))
			{
				if (string.IsNullOrEmpty(NUser_AuthDetail_obj.DetailName))
				{
					NUser_AuthDetail_obj.DetailName = "";
				}
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
				}
				sqlSet.Append("[DetailName]=@DetailName");
				cmd.Parameters.AddWithValue("@DetailName", NUser_AuthDetail_obj.DetailName);
				cmd.Parameters["@DetailName"].SqlDbType = System.Data.SqlDbType.NVarChar;
			}
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[AllowList]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[AllowSync]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[AllowRoll]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[SiteName]"))
			{
				if (string.IsNullOrEmpty(NUser_AuthDetail_obj.SiteName))
				{
					NUser_AuthDetail_obj.SiteName = "";
				}
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[IsDir]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
				}
				sqlSet.Append("[IsDir]=@IsDir");
				cmd.Parameters.AddWithValue("@IsDir", NUser_AuthDetail_obj.IsDir);
				cmd.Parameters["@IsDir"].SqlDbType = System.Data.SqlDbType.Int;
			}
			if(NUser_AuthDetail_obj.ModifiedColumns.Contains("[IsDel]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if (NUser_AuthDetail_obj.CreateTime != null && NUser_AuthDetail_obj.CreateTime > DateTime.MinValue && NUser_AuthDetail_obj.ModifiedColumns.Contains("[CreateTime]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
		...
		/// <returns></returns>
		public static int Update(Trans.Db.Model.NUser_AuthDetail NUser_AuthDetail_obj, bool IsRowLock)
		{
			if(NUser_AuthDetail_obj.ModifiedColumns.Count <= 0)
			{
				return -1;
			...
			if (IsRowLock)
			{
				sql = "UPDATE NUser_AuthDetail WITH(ROWLOCK) SET {0} WHERE {1}";
			}
			else
			{
				sql = "UPDATE NUser_AuthDetail SET {0} WHERE {1}";
			}
			SqlCommand cmd = new SqlCommand();
			cmd.Parameters.AddWithValue("@ID", NUser_AuthDetail_obj.ID);
			cmd.Parameters["@ID"].SqlDbType = System.Data.SqlDbType.Int;
