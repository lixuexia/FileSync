...
namespace Trans.Db.Data
{
	...
	public class NUser_Info
	{
		...
		public static DataTable GetTable(string sqlWhere = "", string sqlSort = "", string sqlCols = "*", object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			...
			StringBuilder sql = new StringBuilder("SELECT " + sqlCols + " FROM [NUser_Info](NOLOCK)");
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_Info> GetList(string sqlWhere = "", string sqlSort = "", object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			DataTable table = GetTable(sqlWhere, sqlSort, "*", ParamsList, ReadOnlyDataSource);
			List<Trans.Db.Model.NUser_Info> rtnObjList = new List<Trans.Db.Model.NUser_Info>();
			if (table != null && table.Rows.Count > 0)
			{
				foreach (DataRow dr in table.Rows)
				{
					rtnObjList.Add(new Trans.Db.Model.NUser_Info(dr));
				}
			}
			return rtnObjList;
		}
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_Info> GetList(string sqlWhere, string sqlSort, string sqlCols, int pageIndex, int pageSize, out int recordCount, object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			long LRecordCount = 0;
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_Info> GetList(string sqlWhere, string sqlSort, string sqlCols, int pageIndex, int pageSize, out long recordCount, object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			long LRecordCount = 0;
			DataTable table = GetTable(sqlWhere, sqlSort, sqlCols, pageIndex, pageSize, out LRecordCount, ParamsList, ReadOnlyDataSource);
		...
		#region 根据主键:UserId 查询，返回对象列表
		/// <summary>
		/// 根据主键:UserId 查询，返回对象列表
		/// </summary>
		/// <param name="UserId"></param>
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static List<Trans.Db.Model.NUser_Info> GetListByUserId(Int32 _UserId, bool ReadOnlyDataSource = true)
		{
			DataTable table = GetTableByUserId(_UserId, ReadOnlyDataSource);
			List<Trans.Db.Model.NUser_Info> objList = new List<Trans.Db.Model.NUser_Info>();
			if (table != null && table.Rows.Count > 0)
			{
				foreach (DataRow dr in table.Rows)
				{
					objList.Add(new Trans.Db.Model.NUser_Info(dr));
				}
			}
			return objList;
		}
		...
		{
			StringBuilder sql = new StringBuilder("SELECT TOP 1 " + sqlCol + " FROM NUser_Info(NOLOCK)");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(" WHERE ").Append(sqlWhere);
			}
		...
		{
			StringBuilder sql = new StringBuilder("SELECT COUNT(1) FROM NUser_Info(NOLOCK) WHERE 1=1");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
		...
		public static DataTable GetTable(string sqlWhere, string sqlSort, string sqlCols, int pageIndex, int pageSize, out long recordCount, object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			...
			{
				sqlCols = "UserId,UserName,Password,RefUserId,Status,IsDel,CreateTime";
			}
			...
			sql.Append("FROM NUser_Info(NOLOCK)");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(" WHERE ").Append(sqlWhere);
			}
			...
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(";SELECT COUNT(1) FROM NUser_Info(NOLOCK) WHERE ").Append(sqlWhere);
			}
			else
			{
				sql.Append(";SELECT COUNT(1) FROM NUser_Info(NOLOCK)");
			}
		...
		/// <param name="ReadOnlyDataSource">是否使用只读数据源</param>
		/// <returns></returns>
		public static Trans.Db.Model.NUser_Info Get(string sqlWhere, string sqlSort = "", object[] ParamsList = null, bool ReadOnlyDataSource = true)
		{
			StringBuilder sql = new StringBuilder("SELECT TOP 1 * FROM NUser_Info(NOLOCK)");
			if (!string.IsNullOrEmpty(sqlWhere))
			{
				sql.Append(" WHERE ").Append(sqlWhere);
			}
			...
			{
				return new Trans.Db.Model.NUser_Info(DT.Rows[0]);
			}
			else
			{
				return new Trans.Db.Model.NUser_Info();
			}
		}
		#endregion
		#endregion
		...
		public static int DBDelete(string sqlWhere, object[] ParamsList = null, bool IsRowLock = true)
		{
			...
			if (IsRowLock)
			{
				deleteSql = "DELETE FROM NUser_Info WITH(ROWLOCK) WHERE ";
			}
			else
			{
				deleteSql = "DELETE FROM NUser_Info WHERE ";
			}
		...
		{
			StringBuilder sql = new StringBuilder("UPDATE NUser_Info SET IsDel=1 WHERE ");
			sql.Append(sqlWhere);
			SqlCommand cmd = BuildCommand(sql.ToString(), ParamsList);
			return new Trans.Db.DBUtility.DBHelper(false).ExecNonQuery(cmd);
		}
		...
		{
			StringBuilder sql = new StringBuilder("UPDATE NUser_Info SET ");
		...
		/// </summary>
		/// <param name="NUser_Info_obj"></param>
		/// <returns></returns>
		public static int Update(Trans.Db.Model.NUser_Info NUser_Info_obj)
		{
			if(NUser_Info_obj.ModifiedColumns.Count <= 0)
			{
				return -1;
			}
			StringBuilder sqlSet = new StringBuilder();
			StringBuilder sqlWhere = new StringBuilder();
			string sql = "UPDATE NUser_Info SET {0} WHERE {1}";
			SqlCommand cmd = new SqlCommand();
			cmd.Parameters.AddWithValue("@UserId", NUser_Info_obj.UserId);
			...
			if(NUser_Info_obj.ModifiedColumns.Contains("[UserName]"))
			{
				if (string.IsNullOrEmpty(NUser_Info_obj.UserName))
				{
					NUser_Info_obj.UserName = "";
				}
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
				}
				sqlSet.Append("[UserName]=@UserName");
				cmd.Parameters.AddWithValue("@UserName", NUser_Info_obj.UserName);
				cmd.Parameters["@UserName"].SqlDbType = System.Data.SqlDbType.NVarChar;
			}
			if(NUser_Info_obj.ModifiedColumns.Contains("[Password]"))
			{
				if (string.IsNullOrEmpty(NUser_Info_obj.Password))
				{
					NUser_Info_obj.Password = "";
				}
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
				}
				sqlSet.Append("[Password]=@Password");
				cmd.Parameters.AddWithValue("@Password", NUser_Info_obj.Password);
				cmd.Parameters["@Password"].SqlDbType = System.Data.SqlDbType.NVarChar;
			}
			if(NUser_Info_obj.ModifiedColumns.Contains("[RefUserId]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
				}
				sqlSet.Append("[RefUserId]=@RefUserId");
				cmd.Parameters.AddWithValue("@RefUserId", NUser_Info_obj.RefUserId);
				cmd.Parameters["@RefUserId"].SqlDbType = System.Data.SqlDbType.Int;
			}
			if(NUser_Info_obj.ModifiedColumns.Contains("[Status]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if(NUser_Info_obj.ModifiedColumns.Contains("[IsDel]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
			...
			if (NUser_Info_obj.CreateTime != null && NUser_Info_obj.CreateTime > DateTime.MinValue && NUser_Info_obj.ModifiedColumns.Contains("[CreateTime]"))
			{
				if (sqlSet.Length > 0)
				{
					sqlSet.Append(",");
		...
		/// <returns></returns>
		public static int Update(Trans.Db.Model.NUser_Info NUser_Info_obj, bool IsRowLock)
		{
			if(NUser_Info_obj.ModifiedColumns.Count <= 0)
			{
				return -1;
			...
			if (IsRowLock)
			{
				sql = "UPDATE NUser_Info WITH(ROWLOCK) SET {0} WHERE {1}";
			}
			else
			{
				sql = "UPDATE NUser_Info SET {0} WHERE {1}";
			}
			SqlCommand cmd = new SqlCommand();
			cmd.Parameters.AddWithValue("@UserId", NUser_Info_obj.UserId);
			cmd.Parameters["@UserId"].SqlDbType = System.Data.SqlDbType.Int;
